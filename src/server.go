...
func Listen(handler func(req Request), listen string, certFile string, keyFile string) {
    ...
    tcpSocket, _ := net.Listen("tcp", listen)

    for {
        tcpConn, _ := tcpSocket.Accept()
        go parse(tcpConn, tlsConfig, handler)
    }
}
...
const BadRequest string = "HTTP/1.1 400 Bad Request\r\n"

func noTLS(conn net.Conn) {
    conn.Write([]byte(BadRequest + "\r\nplease use https"))
    conn.Close()
}

func parse(tcpConn net.Conn, tlsConfig *tls.Config, handler func(req Request)) {

    buffConn := newBufferedConn(tcpConn)
    tlsConn := tls.Server(buffConn, tlsConfig)

    peek, _ := buffConn.Peek(1)
    ...
    message := bufio.NewReader(tlsConn)
